...
- **CORS Atómico (Manual Header Injection)**: Se eliminó el paquete `cors` y se reemplazó por un middleware manual que inyecta los headers `Access-Control-Allow-Origin` (fijo a `wadi-wxg7.onrender.com`) y `Access-Control-Allow-Credentials`. Además, intercepta manualmente el método `OPTIONS` respondiendo `204 immediately`. Esto garantiza que los headers se envíen sí o sí, sin depender de lógica de librerías externas.
- **Graceful Shutdown**: Se añadió manejo de señales `SIGTERM` y `SIGINT` en la API. Esto permite que Render apague el servicio de manera ordenada (Zero-downtime deploy), cerrando el servidor HTTP correctamente antes de matar el proceso.
- **Frontend Health Check**: Se creó el archivo `apps/frontend/public/health` (con contenido "ok") para asegurar que Render reciba un `200 OK` en la ruta `/health` al servir la aplicación estática, evitando errores de "Deploy Failed".
