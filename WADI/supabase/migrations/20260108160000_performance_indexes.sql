...
DO $$ BEGIN -- Conversations & Messages (Core Chat)
...
-- Workspace Management (Fixed: Checking for owner_id vs user_id)
IF EXISTS (
    SELECT 1
    FROM information_schema.columns
    WHERE table_name = 'workspaces'
        AND column_name = 'user_id'
) THEN CREATE INDEX IF NOT EXISTS idx_workspaces_user_id ON public.workspaces(user_id);
END IF;
IF EXISTS (
    SELECT 1
    FROM information_schema.columns
...
) THEN CREATE INDEX IF NOT EXISTS idx_workspaces_owner_id ON public.workspaces(owner_id);
END IF;
